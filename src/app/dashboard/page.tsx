...
export default async function DashboardPage() {
  ...
  const [activities, challenges] = await Promise.all([getMyActivities(5), getMyChallenges()])
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <Badge
            variant="outline"
            className="px-4 py-1 text-sm bg-primary/10 text-primary border-primary/20"
          >
            <Leaf className="h-4 w-4 mr-2" /> {getLevelName(profile?.level ?? 1)} (Level{' '}
            {profile?.level ?? 1})
          </Badge>
          <LogActivityModal />
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {profile?.total_co2_saved_kg.toFixed(1) ?? '0.0'} kg
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {completedQuests} / {challenges.length}
            </div>
            ...
            {activities.length === 0 ? (
              ...
              activities.map((act) => (
                <div key={act.id} className="flex items-center gap-4">
                  <div
                    className={`${CATEGORY_BG[act.category] ?? 'bg-white/5'} p-2 rounded-full shrink-0`}
                  >
                  ...
                  <div className="space-y-0.5 flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatDistanceToNow(act.created_at)}
                    </p>
                  </div>
                  <div
                    className={`font-medium text-sm shrink-0 ${act.co2_kg < 0 ? 'text-emerald-500' : 'text-destructive'}`}
                  >
                    {act.co2_kg < 0 ? '-' : '+'}
                    {Math.abs(act.co2_kg)} kg
                  </div>
                </div>
              ))
            )}
