...
import { LuUtensils as Utensils, LuLeaf as Leaf, LuCar as Car, LuZap as Zap, LuShoppingBag as ShoppingBag, LuPlaneTakeoff as Plane, LuMonitor as Monitor } from "react-icons/lu"
...
export default async function ActivitiesPage() {
  ...
  const totalSavings = activities.filter(a => a.co2_kg < 0).reduce((s, a) => s + Math.abs(a.co2_kg), 0)
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Detailed view of your tracked actions and their environmental impact.</p>
        </div>
        <LogActivityModal />
      </div>
        ...
        <CardContent className="space-y-3">
          {activities.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm mt-1">Use the &quot;Log Activity&quot; button to get started.</p>
            </div>
          ) : (
            activities.map((act) => (
              <div key={act.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-muted/30 transition-colors">
                <div className={`${CATEGORY_BG[act.category] ?? 'bg-white/5'} p-2.5 rounded-xl shrink-0`}>
                ...
                <div className="space-y-0.5 flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-2">
                    <span className="capitalize">{CATEGORY_LABEL[act.category] ?? act.category}</span>
                    <span>·</span>
                    <span>{formatDistanceToNow(act.created_at)}</span>
                  </p>
                </div>
                <div className={`font-semibold text-sm shrink-0 ${act.co2_kg < 0 ? 'text-emerald-500' : 'text-red-400'}`}>
                  {act.co2_kg < 0 ? '−' : '+'}{Math.abs(act.co2_kg)} kg CO₂
                </div>
              </div>
            ))
          )}
