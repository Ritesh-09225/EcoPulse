...
export default async function LeaderboardPage() {
  const [leaderboard, myProfile] = await Promise.all([
    getLeaderboard(),
    getMyProfile(),
  ])
        ...
        <TabsContent value="global" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>The most sustainable individuals worldwide, ranked by Eco Points.</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-4 p-3 rounded-xl border transition-colors ${
                          isMe ? 'bg-primary/10 border-primary/30' : 'bg-card hover:bg-muted/40 border-border'
                        }`}
                      >
                        {/* Rank */}
                        ...
                        <div className="flex-1 space-y-0.5 min-w-0">
                          <p className="text-sm font-medium leading-none flex items-center gap-2 flex-wrap">
                            ...
                            {isMe && <Badge variant="default" className="text-[10px] h-4 px-1 shrink-0">YOU</Badge>}
                          </p>
                          <p className="text-xs text-muted-foreground">{getLevelName(entry.level)} · {entry.total_co2_saved_kg.toFixed(1)} kg saved</p>
                        </div>
