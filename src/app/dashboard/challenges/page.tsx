...
export default async function ChallengesPage() {
  ...
  const [userChallenges, achievements] = await Promise.all([getMyChallenges(), getMyAchievements()])
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Complete challenges to earn points, unlock levels, and save the planet.
        </p>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 space-y-2 text-center md:text-left">
              ...
              <p className="text-muted-foreground">
                {current.toLocaleString()} / {required.toLocaleString()} Eco Points
              </p>
              <Progress value={percent} className="h-3 w-full max-w-md mx-auto md:mx-0" />
              <p className="text-xs text-muted-foreground">
                {(required - current).toLocaleString()} points to next level: &apos;{nextLevelName}
                &apos;
              </p>
            </div>
            ...
            {active.length === 0 ? (
              ...
              active.map((uc) => {
                const progressPct = Math.min(
                  100,
                  Math.round((uc.progress / uc.challenge.target_value) * 100),
                )
                return (
                  <div key={uc.id} className="space-y-2">
                    <div className="flex justify-between items-center text-sm">
                      <span className="font-medium">
                        {uc.challenge.title} ({uc.challenge.type})
                      </span>
                      <span className="text-primary font-bold">
                        +{uc.challenge.points_reward} pts
                      </span>
                    </div>
                ...
                {completed.slice(0, 3).map((uc) => (
                  <div key={uc.id} className="flex justify-between items-center text-sm">
                    <span className="flex items-center gap-2 text-muted-foreground line-through">
                      <CheckCircle2
                        className="h-3.5 w-3.5 text-emerald-500 no-underline"
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              {achievements.map(({ achievement, earned }) => (
                ...
                >
                  <div
                    className={`h-12 w-12 rounded-full flex items-center justify-center ${COLOR_MAP[achievement.color] ?? 'bg-muted text-muted-foreground'}`}
                  >
